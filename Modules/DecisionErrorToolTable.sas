...
Description:	This macro creates an output table for the DecisionErrorTool.
...
Usage:			The input dataset should be the results dataset output from DecisionErrorTool.

				Example call:
				%DecisionErrorToolTable( lib = out01, 
                                         din = results, 
                                         title = Table 1. Simulated Decision Metrics., -9
                                         errorcode = 9999 );
 );
********************************************************************************************/
...
%macro DecisionErrorToolTable( lib, din, title, errorCode )/ store source ;
	...
	/*hypothesis test rejected in the correct direction, failed to reject, or rejected in the wrong direction, respectively*/
	/*do not include realizations where bias correction failed*/
	/*those will be output as a separate table*/
	...
	/*calculate the t1 error, correct rejection fraction, and wrong rejection fraction for the complete analysis*/
	data true;

		set directionCompNull ( in = t1 rename = ( null_directionCompleteHypTest = direction )
                                where = ( direction = 1 ) keep = null_directionCompleteHypTest percent )
			directionCompAlt  ( in = crf rename = ( alt_directionCompleteHypTest = direction )
                                where = ( direction = 1 ) keep = alt_directionCompleteHypTest percent )
			directionCompAlt  ( in = wrf rename = ( alt_directionCompleteHypTest = direction )
                                where = ( direction = -1 ) keep = alt_directionCompleteHypTest percent );
	...
	/*calculate the t1 error, correct rejection fraction, and wrong rejection fraction for the observed analysis*/
	data obs;

		set directionObsNull ( in = t1 rename = ( null_directionObservedHypTest = direction )
                                where = ( direction = 1 ) keep = null_directionObservedHypTest percent )
			directionObsAlt  ( in = crf rename = ( alt_directionObservedHypTest = direction )
                                where = ( direction = 1 ) keep = alt_directionObservedHypTest percent )
	...
	/*calculate the t1 error, correct rejection fraction, and wrong rejection fraction for the corrected analysis*/
	data corr;

		set directionCorrNull ( in = t1 rename = ( null_directionCorrectedHypTest = direction )
                                where = ( direction = 1 ) keep = null_directionCorrectedHypTest percent )
			directionCorrAlt  ( in = crf rename = ( alt_directionCorrectedHypTest = direction )
                                where = ( direction = 1 ) keep = alt_directionCorrectedHypTest percent )
			directionCorrAlt  ( in = wrf rename = ( alt_directionCorrectedHypTest = direction )
                                where = ( direction = -1 ) keep = alt_directionCorrectedHypTest percent );
	...
	/*merge datasets*/
	/*create order variable so the observations can be properly ordered when we make the report*/
	data table;
	...
	/*sort the table into the proper order (t1 error first, crf second, wrf third*/
	...
	proc report data = table nowd style( report ) = { font_face = 'Times' font_size = 12pt frame = hsides rules = groups }
		...
		define propTrue / 		format = 5.2 "True" 
	                       		center style( column ) = [ just = d ] style( column ) = { cellwidth = 1in };
		define propObs /   		format = 5.2 "Standard" center style( column ) = [ just = d ]
	                      		style( column ) = { cellwidth = 1in };
		define propCorr /  		format = 5.2 "Corrected" center style( column ) = [ just = d ] style( column ) = { cellwidth = 1in };
