...
%DecisionErrorSimulator( -99999, 1, 50000, .000001, 500, { 65 59 }, vector = ( { .01, .025, .05, .075, .1, .125, .15, .175, .2, .225, .25 } )); 
...
%macro BiasCorrectionTool06( ExampleDataset, errorCode, cutOffT1T2 ) / store source; 
...
/* ***********************************  Simulates the dataset and bias corrects it. *********************************** */
			...
			start decisionError;
				...
				tolerance = .000001;				
					...
					use out01.&ExampleDataset;

					read all var _ALL_ into matrix;

					close out01.&ExampleDataset;
...
/* ************************************ Create a matrix to display results ******************************************************** */         
					...
					/* If there are not 107 columns, then add enough columns to make it 107 */
					/* so that the variable names will align properly with the initialized SAS dataset */
