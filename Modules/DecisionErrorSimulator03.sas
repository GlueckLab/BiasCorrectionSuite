...
%macro DecisionErrorSimulator03( seed, errorCode, numberOfrealizations, sampleSize, tolerance, maxIterations, meanOfTest1ScoresCases, meanOfTest2ScoresCases, stdOfTest1ScoresCases, stdOfTest2ScoresCases, correlationT1T2ForCases, meanOfTest1ScoresNonCases,
                         meanOfTest2ScoresNonCases, stdOfTest1ScoresNonCases, stdOfTest2ScoresNonCases, correlationT1T2ForNonCases, diseasePrevalence, cutOffT1T2, rateOfSignsSymptoms ) / store source; 
...
/* **************************** Define Input Parameters ************************************** */
	    ...
	    quadrantMLENonCases				length = 8 label = "Quadrant Containing the MLE for Non-Cases"
...
/* ***********************************  Simulates the dataset and bias corrects it. *********************************** */
			...
			start analysis;
				...
				if &meanOfTest1ScoresCases >= cutoffs[ , 1 ] & &meanOfTest2ScoresCases >= cutoffs[ , 2 ] then quadrantMLECases = 1;
					else if &meanOfTest1ScoresCases >= cutoffs[ , 1 ] & &meanOfTest2ScoresCases < cutoffs[ , 2 ] then quadrantMLECases = 2;
						else if &meanOfTest1ScoresCases < cutoffs[ , 1 ] & &meanOfTest2ScoresCases >= cutoffs[ , 2 ] then quadrantMLECases = 3;
							else quadrantMLECases = 4;

				if &meanOfTest1ScoresNonCases >= cutoffs[ , 1 ] & &meanOfTest2ScoresNonCases >= cutoffs[ , 2 ] then quadrantMLENonCases = 1;
					else if &meanOfTest1ScoresNonCases >= cutoffs[ , 1 ] & &meanOfTest2ScoresNonCases < cutoffs[ , 2 ] then quadrantMLENonCases = 2;
						else if &meanOfTest1ScoresNonCases < cutoffs[ , 1 ] & &meanOfTest2ScoresNonCases >= cutoffs[ , 2 ] then quadrantMLENonCases = 3;
...
/*					&changer = &vector[ vectorRow, ];*/
					...
					do rep = 1 to &numberOfrealizations;
						...
						free / errCode cumErrors pastErrors
			                   ...
			                   meanOfTest1ScoresCases meanOfTest2ScoresCases meanOfTest1ScoresNonCases meanOfTest2ScoresNonCases stdOfTest1ScoresCases stdOfTest2ScoresCases stdOfTest1ScoresNonCases stdOfTest2ScoresNonCases
							   rateSignsSymptoms diseasePrevalence cutoffs populationRhoCases populationRhoNonCases 
							   populationAUCT1 populationAUCT2 populationDeltaAUC quadrantMLECases quadrantMLENonCases
							   ncat vectorRow rep;
