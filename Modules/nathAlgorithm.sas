...
proc iml;
	...
	*summary stats matrix is the input for this next step;
	start nathAlgorithm( errCode, cumErrors, tolerance, maxIterations, errorCode, targetq, co1, co2, mean1, mean2, std1, std2, corr );

		free / errCode cumErrors tolerance maxIterations errorCode targetq co1 co2 mean1 mean2 std1 std2 corr;
		...
		*if the target quadrant is not specified or is not in the right range then do not start;
		if ( targetq ^= 1 && targetq ^= 2 && targetq ^= 3 && targetq ^= 4 ) then do;
			...
			*if starting values are missing, error-coded, or out of range then do not start;
			if ( mean1 = errorCode | mean2 = errorCode | std1 = errorCode | std2 = errorCode | corr = errorCode |
	             mean1 = . | mean2 = . | std1 = . | std2 = . | corr = . | 
				...
				*stop loop if the difference between estimates is greater than or equal to the defined tolerance;
				*stop loop if previous iteration had an error;
				do while ( diff > tolerance & outcome = 0 ); 
						...
						*Calculate G, P, Q, and A using eta, xi, rho values from previous iteration (or starting values);
						*Calculate G;
						*this changes depending on truncated region;
						if targetq = 1 then G = 1 - probnorm( xi ) - probnorm( eta ) + probbnrm( xi, eta, rho0 );
						...
						*if G <= 0 then that means there is not enough information in the truncated region to form nath estimates;
						if G <= 0 then do;
								outcome = 3;
								mu1 = errorCode;
								mu2 = errorCode;
								...
								*Calculate updated parameters;
								m = 1 + xi * Phat + rho0**2 * ( Ahat + eta * Qhat ) - ( Phat + rho0 * Qhat )**2;
						
								n = 1 + eta * Qhat + rho0**2 * ( Ahat + xi * Phat ) - ( Qhat + rho0 * Phat )**2;
								...
								if ( m <= 0 | n <= 0 ) then do;
										...
										*return an error if values are too large or small;
										if ( logTestSig1 <= -706 | logTestSig1 >= 706 |
								 		logTestSig2 <= -706 | logTestSig2 >= 706 ) 
					        			then do;
											outcome = 5;
											mu1 = errorCode;
											mu2 = errorCode;
											...
											*1e307 is the largest number iml will hold in memory;
											*it is easiest to test the size of a value by taking the log;
											*in order to take the log, we must make a test variable that is always positive;
											*log( 1e307 ) is approx 706;
											*log( 1e-307 ) is approx -706;
											...
											*return an error if values are too large or small;
						        			...
						        			then do;
													...
													rho = corr * std1 * std2 / sig1 / sig2 / ( 1 + xi * Phat 
										  			+ eta * Qhat + Ahat - ( Phat + rho0 * Qhat ) * ( Qhat + rho0 * Phat) / rho0 );
													...
													*return an error if values are too large or small;
													if ( logTestRho <= -706 | logTestRho >= 706 ) 
							        				then do;
															outcome = 7;
															mu1 = errorCode;
															mu2 = errorCode;
															...
															*stop and return error if updated values are missing;
															if ( mu1 = . | mu2 = . | sig1 = . | sig2 = . | rho = . ) then do;
																	outcome = 8;
																	mu1 = errorCode;
																	mu2 = errorCode;
																	sig1 = errorCode;
															...
															*stop and return missing if updated values are out of range;
															else if ( sig1 < 0 | sig2 < 0 | rho <= -1 | rho >= 1 ) then do;
																	outcome = 9;
																	mu1 = errorCode;
																	mu2 = errorCode;
																	sig1 = errorCode;
															...
															*return missing values if loop has gone more than max iterations;
															if ( iteration >= maxIterations && diff > tolerance  ) then do;
																	outcome = 10;
																	mu1 = errorCode;
																	mu2 = errorCode;
																	sig1 = errorCode;
													...
													end; *new rho is not too large or too small; /* TAKEN */

											end; *new mu1 and mu2 are not too large or too small; /* TAKEN */

									end; *new sig1 and sig2 are not too large or too small; /* TAKEN */
			
							end; *end m and n both > 0; /* TAKEN */

						end; *end G > 0; /* TAKEN */
 
					end; *end do while loop; /* TAKEN */
				
				end; *end starting values in correct range and not missing; /* TAKEN */

			end; *end target quadrant is in range; /* TAKEN */
