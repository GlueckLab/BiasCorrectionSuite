...
%macro DecisionErrorSimAndExDatasets( seed, errorCode, numberOfrealizations, sampleSize, tolerance, maxIterations, meanOfTest1ScoresCases, meanOfTest2ScoresCases, stdOfTest1ScoresCases, stdOfTest2ScoresCases, correlationT1T2ForCases, meanOfTest1ScoresNonCases,
                         meanOfTest2ScoresNonCases, stdOfTest1ScoresNonCases, stdOfTest2ScoresNonCases, correlationT1T2ForNonCases, diseasePrevalence, cutOffT1T2, rateOfSignsSymptoms, changer, vector ); 
...
%macro createExampleDatasetAndVerify( id, co1, co2 );
	...
	data out01.example&id;
		...
		if disease = 1 and ( ss = 1 or t1score >= &co1 or t2score >= &co2 ) then obsDisease = 1;
		else obsDisease = 0;

		keep obsDisease t1score t2score;

	run;

	/*output a text file with a list of the cutoffs*/
	data cutoffs&id;
	...
	ods pdf file = "&OUTPUT_FILE.CutOffs&id..pdf";

	proc print data = cutoffs&id;
	run;
