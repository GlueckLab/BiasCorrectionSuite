...
The BiasCorrectionTool module corrects an input dataset for paired screening trial bias (Glueck,
2009). The input dataset should be a SAS dataset containing one row per participant, a column of 
screening test scores Screening Test 1, a column of screening test scores for Screening Test 2, 
and a column of disease status indicators (1 = observed disease, 0 = no observed disease). The
module outputs the results of a hypothesis test for the difference in the areas under the 
binormal receiver operating characteristic curves for both a standard analysis and the bias-
corrected analysis.  The standard analysis uses the methods of Obuchowski and McClish (1997).
The bias-corrected analysis is described in Ringham et al. (in review).

The BiasCorrectionTool defines a group of submodules, briefly described below: 

standardAnalysis			performs the standard (or observed) analysis
biasCorrectionAnalysis		performs the bias-corrected analysis
nathInAllQuadrants			checks for quadrant with 2 or more observations and calculates 
							maximum likelihood estimates for the mean, variance, and correlation 
							within each quadrant
setErrorCodes 				defines error codes for variables downstream of known errors
performBiasCorrect			bias corrects dataset and outputs results
...
inputData						name of the input dataset, should be saved in "Input" folder
errorCode						code for undefined entries - missing value not allowed, 
                				avoid values that could be misconstrued as a data point 
cutOffT1T2						biopsy thresholds for Screening Test 1 and 2, format as a row
                                matrix like so: { <threshold for Test1> <threshold for Test 2> }

------------------------------------------------------------------------------------------------
										  USAGE 
------------------------------------------------------------------------------------------------

The BiasCorrectionTool module can be run via following command:

%BiasCorrectionTool( Example1, -99999, { 65 59 } ); 

where, the values listed in the command above correspond to the following input parameters:
...
/* **************************** BiasCorrectionTool MACRO DEFINITION  ************************** */
/* ******************************************************************************************** */

%macro BiasCorrectionTool( inputData, errorCode, cutOffT1T2 ) / store source; 
...
/* ********************* create empty file with preferred characteristics ********************* */
	...
	data out01.resultsBC ( compress = no );
...
/* ********************************* Define Input Parameters ********************************** */
	    ...
	    stdT1ObsCases          			length = 8 label = "Test 1 Standard Deviation for 
															Observed Cases"
	    stdT2ObsCases					length = 8 label = "Test 2 Standard Deviation for 
															Observed Cases"
	    ...
	    stdT2ObsNonCases				length = 8 label = "Test 2 Standard Deviation for Observed 
															Non-Cases"
	    corrObsNonCases					length = 8 label = "Correlation Between Observed 
															Non-Cases"
		numberObsCasesQuadrant1			length = 8 label = "Number of Observed Cases in Quadrant 
															1"
	    ...
	    corrObsCasesQuadrant1	    	length = 8 label = "Correlation Between Observed Cases in 
															Quadrant 1"
		numberObsCasesQuadrant2			length = 8 label = "Number of Observed Cases in Quadrant 
															2"
	    ...
	    corrObsCasesQuadrant2	    	length = 8 label = "Correlation Between Observed Cases in 
															Quadrant 2"
		numberObsCasesQuadrant3			length = 8 label = "Number of Observed Cases in Quadrant 
															3"
	    ...
	    corrObsCasesQuadrant3	    	length = 8 label = "Correlation Between Observed Cases in 
															Quadrant 3"
		numberObsCasesQuadrant4			length = 8 label = "Number of Observed Cases in Quadrant 
															4"
	    ...
	    corrObsCasesQuadrant4	    	length = 8 label = "Correlation Between Observed Cases in 
															Quadrant 4"
		numberObsCasesQ123				length = 8 label = "Number of Observed Cases in Quadrants 
															1, 2, and 3 Together"
		...
		correctedNumberCases			length = 8 label = "Corrected Number of Cases Based On 
															Nath"
		correctedNumberNonCases  		length = 8 label = "Corrected Number of Non-Cases Based 
															On Nath"
		...
		correctedReject					length = 8 label = "Indicator that Delta AUC Hypothesis 
															Test Calculated From Corrected 
															Summary Stats for Cases / Observed 
															for Non-Cases Rejected (1 = reject, 
															0 = fail to reject)"
...
/* ********************* Free variables that do not change within the do loop ***************** */
		...
		free errCode cumErrors maxIterations tolerance sampleSize meanOfTest1ScoresCases 
             meanOfTest2ScoresCases meanOfTest1ScoresNonCases meanOfTest2ScoresNonCases 
             stdOfTest1ScoresCases stdOfTest2ScoresCases stdOfTest1ScoresNonCases 
             stdOfTest2ScoresNonCases rateSignsSymptoms diseasePrevalence cutoffs 
             populationRhoCases populationRhoNonCases populationAUCT1 populationAUCT2 
             populationDeltaAUC quadrantContainingMLECases quadrantContainingMLENonCases;
...
/* Set an error code for variables based on the error encountered at several steps in the       */
/* performBiasCorrect module                                                                    */
			...
			start setErrorCodes; /* start module */
				...
				if (numberObsCasesQuadrant1 < 2 & flag = 1) then do;
					...
					weightedEstimatesRow = &errorCode || &errorCode || &errorCode || 
                                           &errorCode || &errorCode;
					correctedEstimatesRow = &errorCode || &errorCode || &errorCode || 
                                            &errorCode || &errorCode;
...
/* Checks to see if the quadrants are empty. If so, calculates summary stats for only non-empty */
/* quadrants                                                                                    */
			...
			start nathInAllQuadrants;
				...
				if numberObsCasesQ123 < 2 then do;
					...
					covMatrixObsCasesQuadrant4 = cov( obsCasesQuadrant4[ , 1 ] || 
                                                      obsCasesQuadrant4[ , 2 ] );
					...
					if numberObsCasesQuadrant1 < 2 then do;
						...
						covMatrixObsCasesQuadrant1 = cov( obsCasesQuadrant1[ , 1 ] || 
                                                          obsCasesQuadrant1[ , 2 ] );
					...
					if numberObsCasesQuadrant2 < 2 then do;
						...
						covMatrixObsCasesQuadrant2 = cov( obsCasesQuadrant2[ , 1 ] || 
                                                     obsCasesQuadrant2[ , 2 ] );
					...
					if numberObsCasesQuadrant3 < 2 then do;
						...
						covMatrixObsCasesQuadrant3 = cov( obsCasesQuadrant3[ , 1 ] || 
                                                     obsCasesQuadrant3[ , 2 ] );
...
/* Performs Standard Analysis on the data                                                       */
			...
			start standardAnalysis;
				...
				/*use only element 12 of the 2 x 2 covariance matrix*/
				corrObsCases = covMatrixObsCases[ 1, 2 ] / 
                               stdT1ObsCases / 
                               stdT2ObsCases;
				...
				observedAUCT1 = calculateAUC( errcode, cumErrors, meanT1ObsCases, 
                                              meanT1ObsNonCases, stdT1ObsCases, stdT1ObsNonCases );
				observedAUCT2 = calculateAUC( errcode, cumErrors, meanT2ObsCases, 
                                              meanT2ObsNonCases, stdT2ObsCases, stdT2ObsNonCases );
...
/* Bias Corrects the data 				                                                        */
			...
			start biasCorrectionAnalysis;
				...
				/*if the Nath estimates are missing for all 4 quadrants then set remaining values 
				  to missing*/
				...
				/* ***************                           *************** */
				    ...
				    bivariateNormalProbabilitiesNath = 
                       estimateBivNormProbabilities( errcode, cumErrors, cutoffs[ , 1 ], 
                                                     ...
                                                     nathEstimatesRow[ , 6 ], nathEstimatesRow[ , 7 ], 
                                                     nathEstimatesRow[ , 8 ], nathEstimatesRow[ , 9 ], 
                                                     &errorCode );
					...
					/*we will use these estimates as the corrected estimates*/
						...
						correctedNumberCasesNonCases = 
                           calcCorrectedNumCasesNonCases( errcode, cumErrors, 
                                                          bivNormProbsWeighted[ , 1 ], sampleSize, 
                                                          numberObsCasesQ123 );
...
/* *************************** performBiasCorrect MODULE DEFINITION *************************** */ 
/* Bias corrects the input dataset                                                              */
			
			start performBiasCorrect;
				...
				free / errCode cumErrors maxIterations tolerance sampleSize;
				
				use in01.&inputData;					

				read all var _ALL_ into matrix;

				close in01.&inputData;
				
				cutoffs = {65 59};

				sampleSize = nrow(matrix);
				...
				/*if not, we will not calculate any summary statistics and we will return an 
				  error*/

				/*find the locations of each observation in each of the groups*/
				/*if any of the locations are empty, write an error*/
				/*Observed cases: disease = 1 and at least one test score is above the threshold 
				  or ss = 1*/
...
/* **************************** Create a matrix to output results ***************************** */         
				...
				output =    obsError || nathError || weighted || correctedNumberCasesError || 
							...
							outcomeNathQuad1 || outcomeNathQuad2 || 
                            outcomeNathQuad3 || outcomeNathQuad4 ||
				...
				if cumErrors >= 1 then resumeError = 1;
				else resumeError = 0;

				/* The output matrix should have 107 columns */
				/* If there are not 107 columns, then add enough columns to make it 107 so that 
				   the variable names will align properly with the initialized SAS dataset */
				numColumns = ncol(output);

				if ( ncol( output ) ^= 107 ) then do;

						columnError = 1;
						additionalColumnsNeeded = 107 - ncol( output );
						addColumns = j( 1, additionalColumnsNeeded, &errorCode );
						results = output || addColumns || columnError || 
                                  additionalColumnsNeeded || resumeError || cumErrors;

				end;

				else do;

						results = output || 0 || 0 || resumeError || cumErrors;

				end;

/* ********************************** Open dataset to edit ************************************ */

				edit out01.resultsBC;

				append from results; 
	
				close out01.resultsBC;  
  
				cumErrors = -1;

			finish; /* finish performBiasCorrect module */

/* ****************************** END MODULE performBiasCorect ******************************** */

			libname in01 "..\..\Input";

			run performBiasCorrect;
	...
	%BiasCorrectionToolTable( lib = out01, 
                              din = resultsBC, 
